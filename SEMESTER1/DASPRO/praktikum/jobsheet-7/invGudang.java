      ...
      if (c == 'Y' || c == 'y') {
        ...
        for (int i = 0; i < stok.length; i++) { // repeat per stok
          if ((int) stok[i][0] == matchedQuery) {
            tableRow(new Object[][] { { Integer.toString(i), 13 }, { namaObat[(int) stok[i][0]], 40 },
                { Integer.toString(getStokObat(i)), 17 }, { getExpObat(i), 19 }, { getBatchObat(i), 25 },
                { gudang[(int) stok[i][2]], 15 } });
          }
        }
        tableLine(136);
      }
